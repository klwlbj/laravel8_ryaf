...
namespace App\Utils\Apis\Core;

use Exception;

class AepSdkCore
{
    public static $baseUrl           = "https://2000037412.api.ctwing.cn";
    public static $timeUrl           = "https://2000037412.api.ctwing.cn/echo";
    public static $offset            = 0;
    public static $lastGetOffsetTime = 0;
    ...
    public static function sendSDkRequest($path, $head, $param, $body, $version, $application, $secret, $method = "GET")
    {
        ...
        $header = [
            ...
            "timestamp"   => "" . $timestamp,
            "version"     => $version,
            "signature"   => self::sign($paramTmp, $timestamp, $application, $secret, self::getBytes($body)),
        ];
        if ($head != null) {
     ...
     * @return 签名数据
     */
    public static function sign($param, $timestamp, $application, $secret, $body)
    {
        //将业务数据排序
        ksort($param);

        //写入timestamp、application数据
        $temp = ["application" => $application, "timestamp" => $timestamp];
        $temp = $temp + $param;
        $s    = "";
     ...
     * hmac-sha1编码
     */
    public static function HmacSHA1Encrypt($encryptText, $encryptKey)
    {
